<?php
/**
 * Skill Progress – skills/progress.php
 * Shows daily / weekly / monthly skill completion stats for the logged-in user.
 */
...
switch ($view) {
    ...
    case 'weekly':
        $sql = "SELECT
                ...
                FROM skills
                WHERE user_id = ?
                  AND YEARWEEK(skill_date, 1) = YEARWEEK(CURDATE(), 1)";
        break;
...
<head>
    ...
    <title>Skill Progress – <?= htmlspecialchars($period) ?></title>
...
<div class="card">
    ...
    <?php if ($total === 0): ?>
        ...
        <div class="empty-state">
            ...
            No skill records found for <?= htmlspecialchars($period) ?>.
        </div>
    ...
    <?php else: ?>
        ...
        <div class="progress-wrap"
             ...
             aria-valuemin="0"
             aria-valuemax="100"
             aria-label="Skill completion rate <?= $percent ?>%">
        ...
        <div class="chart-wrap">
            <canvas id="chart" aria-label="Skill progress bar chart"></canvas>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    new Chart(document.getElementById('chart').getContext('2d'), {
        type: 'bar',
        data: {
            labels: ['Practiced', 'Skipped'],
            datasets: [{
                data: [done, missed],
                backgroundColor: ['rgba(244,114,182,0.85)', 'rgba(239,68,68,0.85)'],
                hoverBackgroundColor: ['#f472b6', '#ef4444'],
                borderRadius: 8,
                barThickness: 56,
            }]
        },
